...
import { Sparkles, Mail, Lock, ArrowRight, Loader, Eye, EyeOff } from 'lucide-react';
...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!email || !password || isLoading || isRedirecting) return;
    ...
    <div className="min-h-screen flex items-center justify-center px-6 relative py-12 bg-transparent">
      ...
      <div className="w-full max-w-md bg-white border-3 border-black shadow-neo-lg rounded-3xl p-8 relative z-10">
        ...
        <div className="text-center mb-8">
          ...
          <h2 className="text-2xl md:text-3xl font-black tracking-tight text-black mb-2 leading-none">Selamat Datang Kembali</h2>
          <p className="text-sm font-bold text-black/60 mt-1">Masuk untuk melanjutkan alokasi jadwal cerdas Anda</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div className="space-y-2 text-left">
            <div className="flex justify-between items-center px-1">
              <label className="text-xs font-black text-black uppercase tracking-wider">Kata Sandi</label>
              <Link href="/auth/forgot-password" className="text-xs text-black font-extrabold hover:underline decoration-neoOrange decoration-2">Lupa sandi?</Link>
            </div>
            <div className="relative">
          ...
          >
            {isLoading ? (
              <>
                <Loader className="w-4 h-4 animate-spin" /> Masuk Sesi...
              </>
            ) : (
              <>
        ...
        <p className="text-center text-xs text-black font-bold mt-8">
          Belum memiliki akun?{' '}
          <Link href="/auth/signup" className="text-black font-extrabold hover:underline decoration-neoPink decoration-2">
            Buat akun baru
          </Link>
        </p>
      </div>
