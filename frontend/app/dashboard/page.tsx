...
import AsepAIWorkspace from '../../components/Dashboard/AsepAIWorkspace';
...
import { queueOfflineTask, flushOfflineQueue } from '../../lib/offlineQueue';
...
export default function DashboardPage() {
  ...
  const [activeTab, setActiveTab] = useState<
    | 'overview' | 'list' | 'calendar' | 'analytics'
    | 'integrations' | 'preferences' | 'welearn' | 'siak'
    | 'excuse-letter' | 'profile' | 'billing' | 'asep-ai' | 'asep'
  >('overview');
  ...
  // Listener otomatis untuk mengirim task offline begitu kembali online
  useEffect(() => {
    const handleOnline = async () => {
      const synced = await flushOfflineQueue((taskData) => API.post('/tasks', taskData));
      if (synced > 0) {
        toast.success(`📶 Terhubung kembali! ${synced} tugas offline berhasil tersinkronisasi.`);
        fetchTasks();
      }
    };
  ...
  const handleCreateTask = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (typeof window !== 'undefined' && !navigator.onLine) {
        ...
        toast.info('📶 Sedang Offline — Misi disimpan di perangkat dan akan terkirim otomatis saat online!');
        setNewTitle('');
        setNewDescription('');
        return;
      }
  ...
  const staggeredMenuItems = [
    { label: 'Beranda',            ariaLabel: 'Halaman Beranda',        onClick: () => setActiveTab('overview') },
    { label: 'Misi Utama',         ariaLabel: 'Daftar Misi Tugas',      onClick: () => setActiveTab('list') },
    { label: 'Agenda Kalender',    ariaLabel: 'Kalender Agenda',        onClick: () => setActiveTab('calendar') },
    { label: 'Analitik Performa',  ariaLabel: 'Analisis Performa',      onClick: () => setActiveTab('analytics') },
    { label: 'WeLearn Academic',   ariaLabel: 'Tugas WeLearn',          onClick: () => setActiveTab('welearn') },
    ...
    { label: 'Asep AI Asisten',    ariaLabel: 'Workspace Asep AI',      onClick: () => setActiveTab('asep-ai') },
    { label: 'Profil Pengguna',    ariaLabel: 'Profil Pengguna',        onClick: () => setActiveTab('profile') },
    { label: 'Integrasi Layanan',  ariaLabel: 'Integrasi Kalender',     onClick: () => setActiveTab('integrations') },
    ...
    { label: 'Keluar Sesi',        ariaLabel: 'Keluar Sesi',            onClick: handleLogout },
  ];
  ...
  return (
    <div className="min-h-screen flex flex-row bg-transparent">
      {showOnboarding && (
        <OnboardingWizard onComplete={handleOnboardingComplete} />
      )}

      {/* COLLAPSIBLE SIDEBAR — Desktop (md+) */}
      ...
      <div className="flex-1 flex flex-col min-w-0 pb-32 md:pb-0">
        {/* MOBILE BOTTOM NAVIGATION BAR & PWA INSTALL BANNER */}
        <MobileBottomNav activeTab={activeTab} onTabChange={setActiveTab} />
        <PWAInstallPrompt />

        <main className="flex-grow w-full px-4 md:px-6 pt-20 pb-5 md:pt-8 md:pb-6">
          <AnimatePresence mode="wait">
            ...
            >
              ...
              {activeTab === 'overview' && (
                <OverviewTab onNavigateToTab={(tab) => setActiveTab(tab as any)} tasks={tasks} />
              )}

              {/* Render Tab Asep AI Workspace */}
              {(activeTab === 'asep-ai' || activeTab === 'asep') && (
                <div className="w-full">
                  <AsepAIWorkspace onNavigateToTab={(tab) => setActiveTab(tab as any)} />
                </div>
              )}
      ...
      <MoreDrawer
        ...
        onNavigate={(tab) => { setActiveTab(tab as any); setMoreOpen(false); }}
        onLogout={handleLogout}
        user={user}
      />
      ...
      <CommandPalette
        ...
        onNavigate={(tab) => { setActiveTab(tab as any); setIsCommandPaletteOpen(false); }}
        onSyncWeLearn={syncMoodle}
